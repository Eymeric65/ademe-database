...
the published data actually supports. The result is one JSON committed to the
repository, so the site build stays hermetic -- no R2 credentials at build time
-- and a refresh of the statistics is a diff somebody reviews. See ADR-0044.

It is run by hand after a weekly run, and stamped with the manifest's
`built_at` and `high_water` so the pages can say as of when: the published tree
runs structurally a couple of months behind, because the recent window is paid
(ADR-0039).
"""
